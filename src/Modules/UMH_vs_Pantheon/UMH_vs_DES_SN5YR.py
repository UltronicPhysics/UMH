"""
UMH_vs_DES_SN5YR.py  (UMH vs DES_SN5YR Test)

DES_SN5YR Hubble-diagram comparison of UMH (non-expansion) vs flat ΛCDM.

Author: Andrew Dodge
Date: July 2025

Implements the analysis described in:
  Companion DES-SN5YR validation of:
  A. Dodge, "Pantheon+ and Redshift Observational Analysis of the UMH Redshift Formulation",
  Reports in Advances of Physical Sciences 8 (2026) 2650012,
  DOI: 10.1142/S242494242650012X, and Appendix A.2.7–A.2.8 of
  "The Ultronic Medium Hypothesis (UMH)".

Key features:
  - Uses DES_SN5YR/Dovekie Hubble-diagram sample (N=1820) with the supplied STAT+SYS inverse covariance.
  - Uses the Pantheon+/Cepheid-calibrated α, held fixed in the DES analysis.
  - UMH non-expansion μ(z) with α fixed from Pantheon+/Cepheid calibration;
    β1 and β2 are recovered from DES and then held fixed for the conditional
    final likelihood, in which only M is profiled.
...
def get_default_config():
    ...
    return {
        ...
        "UMH_ALPHA_FIXED": 2.481805e-04,  # Pantheon+/Cepheid calibrated UMH alpha, 1/Mpc
...
def run(config_overrides=None):
    ...
    columns=config["DES_SN5YR_DATA_COLUMNS"]
    panfile=config["DES_SN5YR_DATA_FILE"]

    panfile_bias=config["DES_SN5YR_DATA_BIAS_FILE"]
    panfile_so_bias=config["DES_SN5YR_DATA_BIAS_SO_FILE"]
    ...
    file_root="UMH_vs_SN"
    sub_file_root="UMH_vs_DES_SN5YR"

    title="UMH vs DES_SN5YR"
    file_hdr="UMH_vs_DES_SN5YR"
    ...
    os.makedirs(outdir, exist_ok=True)
    outdir=os.path.join(outdir, sub_file_root)
    os.makedirs(outdir, exist_ok=True)
    ...
    DO_LOWZ_CALIB = False #DO_LOWZ_CALIB = True
    ZMAX_CALIB    = 0.10

    DO_PREWHITEN = False  # Keep False for the published DES precision-matrix path; diagnostic branch is not part of the published run.
    ...
    # --- Load the DES-Dovekie Hubble diagram ---
    df = pd.read_csv(panfile)
    ...
    if missing: raise RuntimeError(f"DES-Dovekie HD file is missing required columns: {missing}")

    # DES-Dovekie_HD.csv is already the ordered Hubble-diagram sample.
    ...
    # Keep variable name mb_corr so the rest of the Pantheon-derived code still works.
    # For DES, this is distance modulus MU, not apparent magnitude m_b_corr.
    mb_corr = df["MU"].to_numpy(float)
    mb_corr_err_diag = df["MUERR"].to_numpy(float)

    ids_subset = df["IDSURVEY"].to_numpy()
    ids_subset = np.where(np.isfinite(ids_subset), ids_subset, -1).astype(int)

    # --- Load DES-Dovekie STAT+SYS inverse covariance from NPZ ---
    # OLD The loader converts DES inverse covariance into ordinary covariance C, because the rest of this script uses C with Cholesky solves.
    # DES provides inverse covariance. Keep it as Cinv throughout this script.
    C = load_des_covariance_from_npz(panfile_bias)
    if C.shape[0] != len_z:
        raise RuntimeError(f"Covariance/data length mismatch: C is {C.shape}, but DES HD has N={len_z}. Make sure you are using DES-Dovekie_HD.csv "
                           f"ordered with the matching STAT+SYS.npz.")
    Csel = C
    #err_plot = np.sqrt(np.diag(Csel))
    #err_plot = np.sqrt(df["MUERR"].to_numpy(float)**2 + df["MUERR_SYS"].to_numpy(float)**2)
    #err_plot = df["MUERR"].to_numpy(float)
    err_plot = np.clip(df["MUERR"].to_numpy(float), 0, 1.0)
    print("MUERR percentiles:", np.nanpercentile(df["MUERR"].to_numpy(float), [0, 50, 90, 99, 100]))
    print("MUERR_SYS percentiles:", np.nanpercentile(df["MUERR_SYS"].to_numpy(float), [0, 50, 90, 99, 100]))
    ...
    def chi2_and_M_best(data_vec, model_mu, Cinv):
        """
        data_vec: observed distance modulus / corrected magnitude
        model_mu: model distance modulus at the same z, before additive M
        Cinv: inverse covariance / precision matrix
        """
    ...
    # --- UMH transmission-scale reporter (chosen length; Planck length by default) ---
    def print_umh_microphysics(a, beta1, beta2, ell_int_mpc=None, label="Planck"):
        # Chosen reference length; Planck length by default.
        if ell_int_mpc is None: ell_int_mpc = 1.616255e-35 / 3.085677581491367e22

        # For the pure UMH law L = ln(1+z) = a*d:
        dL = a * ell_int_mpc

        # Transmission exponent τ(L) = β1 L + β2 L².
        # These are local/first-order scale quantities, not photon-loss probabilities.
        dtau_0 = beta1 * dL
        d_dtau_dL = 2.0 * beta2 * dL
        dtau_dd_0 = a * beta1

        tau_length_0 = np.inf if dtau_dd_0 <= 0 else 1.0 / dtau_dd_0

        print(f"[UMH transmission scales @ {label}] ℓ_ref = {ell_int_mpc:.3e} Mpc")
        print(f"  ΔL = Δln(1+z)          = {dL:.3e}")
        print(f"  first-order Δτ (L≈0)   = {dtau_0:.3e}")
        print(f"  d(Δτ)/dL               = {d_dtau_dL:.3e}")
        print(f"  (dτ/dd)|L=0 = a*β1     = {dtau_dd_0:.3e} 1/Mpc")
        print(f"  first-order τ length   ≈ {tau_length_0:.1f} Mpc")
    ...
    if DO_LOWZ_CALIB:
        ...
        if lowz_out is not None:
            ...
            A_NATIVE = float(config["UMH_ALPHA_FIXED"])
            print(f"[Fixed α] A_NATIVE={A_NATIVE:.6e} 1/Mpc")
    else:
        A_NATIVE = float(config["UMH_ALPHA_FIXED"])
        print(f"[Fixed α] A_NATIVE={A_NATIVE:.6e} 1/Mpc")
        #A_NATIVE = H0 / config["LIGHT_SPEED"]        # ≈ H0/c in 1/Mpc
    ...
    if DO_PREWHITEN:
        ...
        try:
            ...
            r_wh = L_wh.T @ (mb_corr - (mu_noatt + M_tmp))
            mask_lowz = (z < 0.02)
            if mask_lowz.any():
    ...
    def betas_from_residuals_GLS(z, mb_corr, mu_noatt, C, huber_c=1.35, max_iter=30):
        ...
        Lc = L - L0
        A = np.vstack([np.ones_like(Lc), 1.086*Lc, 1.086*(Lc**2)]).T

        # Pre-whiten using inverse covariance:
        # if Cinv = Lch @ Lch.T, then whitened vectors are Lch.T @ vector
        Lch = np.linalg.cholesky(C)
        K = Lch.T

        y = K @ (mb_corr - mu_noatt)
        X = K @ A
    ...
    if DO_CALC_BETAS_FROM_DATA:
        for c in (1.35, 1.50, 1.80, 2.00):
            ...
            Lch = np.linalg.cholesky(Csel)
            K = Lch.T
            X = K @ A
            y = K @ (mb_corr - (mu_noatt + dmu))
        ...
        # IMPORTANT:
        # A_NATIVE is fixed from the Pantheon+/Cepheid calibration.
        # β1 and β2 are recovered from the DES transmission-calibration stage
        # and then held fixed in the conditional final DES likelihood.
        # Only the magnitude/intercept M is profiled there (k=1 for UMH).
        #UMH_BETA1_THEORY, UMH_BETA2_THEORY = betas_jackknife_idsurvey(ids_subset, z, mb_corr, mu_noatt, Csel, huber_c=2.0)
        UMH_BETA1_THEORY, UMH_BETA2_THEORY = betas_from_residuals_GLS(z, mb_corr, mu_noatt, Csel, huber_c=2.0)

        #UMH_BETA1_THEORY = 0.432; UMH_BETA2_THEORY = -0.270 #From Pantheon

        print(f"Recovered from DES transmission calibration: β1={UMH_BETA1_THEORY:.4f}, β2={UMH_BETA2_THEORY:.4f}")
    else: 
        UMH_BETA1_THEORY=0.430923; UMH_BETA2_THEORY=-0.275549
        print(f"Configured fixed values: β1={UMH_BETA1_THEORY:.4f}, β2={UMH_BETA2_THEORY:.4f}")
    ...
    def fit_umh_theory_fixed():
        """
        Use α fixed from the Pantheon+/Cepheid calibration and β1,β2 fixed after
        the DES recovery stage, then profile only M in the conditional final
        likelihood. Returns an 'inc'-shaped dict.
        """
        a = float(A_NATIVE)
        ...
        print(f"[UMH calibrated-fixed] a={a:.9g}, β1={beta1:g}, β2={beta2:g} → χ² = {chi2:.1f}  (k = 1, counting profiled M)")

        # k = 1 because only M is varied/profiled in the conditional final DES likelihood;
        # α is fixed from Pantheon+/Cepheid calibration and β1,β2 are fixed after DES recovery.
    ...
    def slope_gls(x, r, Cinv):
        """
        GLS fit of r = m*x + b using inverse covariance / precision matrix Cinv.
        Returns: m, b, m_err, b_err
        """
        A = np.vstack([x, np.ones_like(x)]).T
        ATA = A.T @ Cinv @ A; ATy = A.T @ Cinv @ r
    ...
    plt.errorbar(z, mb_corr, yerr=err_plot, fmt='.', label="DES_SN5YR (MU)", alpha=0.5)
    parts = [f"a*={a_best:.3g}"]
    ...
    plt.plot(z, residuals_umh_best, '.', alpha=0.5)
    #plt.errorbar(z, residuals_umh_best, yerr=err_plot, fmt='.')
    plt.axhline(0, color='k', linestyle='--')
    plt.xlabel("Redshift z"); plt.ylabel("MU − (μ_UMH(non-exp) + M)")
    plt.title(f"{title}: Residuals (DES_SN5YR − UMH non-exp)")
    ...
    # --- Overlay: DES_SN5YR vs best-fit UMH(non-exp) vs best-fit ΛCDM ---
    plt.figure(figsize=(10,6))
    plt.errorbar(z, mb_corr, yerr=err_plot, fmt='o', ms=3, label='DES_SN5YR (MU)', alpha=0.6)
    ...
    plt.xlabel('Redshift z'); plt.ylabel('Distance Modulus μ')
    plt.title(f"{title}: Best-fit UMH(non-exp) vs ΛCDM vs DES_SN5YR")
    plt.legend(); plt.grid(True); plt.tight_layout()
    ...
    L = np.linalg.cholesky(Csel)
    K = L.T
    w_umh = K @ residuals_umh_best
    w_lcdm = K @ residuals_lcdm_best
    frac_closer = np.mean(np.abs(w_umh) < np.abs(w_lcdm))
    print(f'Fraction with |whitened residual| smaller for UMH: {frac_closer:.3f}')

    plt.figure(figsize=(12,5))
    plt.errorbar(z, residuals_umh_best,  yerr=err_plot, fmt='o', label=f'DES_SN5YR − UMH (a*={a_best:.3g})', alpha=1.0)
    plt.errorbar(z, residuals_lcdm_best, yerr=err_plot, fmt='s', label=f'DES_SN5YR − ΛCDM (Ωm*={Om_best:.3f})', alpha=0.6)
    ...
    ax.errorbar(z, residuals_umh_best,  yerr=err_plot, fmt='o', label=f'DES_SN5YR − UMH (a*={a_best:.3g})',  alpha=1.0)
    ax.errorbar(z, residuals_lcdm_best, yerr=err_plot, fmt='s', label=f'DES_SN5YR − ΛCDM (Ωm*={Om_best:.3f})', alpha=0.6)
    ...
    #plt.plot(z, mu_umh_best  + M_umh_best,  'r-',  label=f'UMH non-exp (a*={a_best:.3g})')
    plt.plot(z, mu_lcdm_best + M_l_best,    'g--', label=f'ΛCDM (Ωm*={Om_best:.3f})')
    regions = [(0.01, 0.10, 'Low-z'), (0.10, 1.15, 'DES/low-z combined')]
